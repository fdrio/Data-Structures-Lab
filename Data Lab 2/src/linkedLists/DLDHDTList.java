...
public class DLDHDTList<E> implements LinkedList<E> {
	...
	public void addNodeBefore(Node<E> target, Node<E> nuevo) {
		// ADD CODE HERE
		addNodeAfter(((DNode<E>) target).getPrev(), nuevo);
	...
	public Node<E> getNodeAfter(Node<E> target)
		...
		if(target==getLastNode()) return null;
		DNode<E> node = (DNode<E>) target;
		return node.getNext(); 
	}
	...
	public Node<E> getNodeBefore(Node<E> target)
			throws NodeOutOfBoundsException {
		// ADD CODE HERE AND MODIFY RETURN ACCORDINGLY
		if(target==getFirstNode()) return null;
		DNode<E> node = (DNode<E>) target;
		return node.getPrev(); 	
		}
	...
	public void removeNode(Node<E> target) {
		// ADD CODE HERE 
		DNode<E> targ = (DNode<E>)target;
		DNode<E> prev = targ.getPrev(), next = targ.getNext();
		prev.setNext(next);
		next.setPrev(prev);
		targ.cleanLinks();
